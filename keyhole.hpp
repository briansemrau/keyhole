...
class Keyhole {
public:
	static void keyhole_tree(const PolyNode& node, Paths& out) {
		if (node.IsHole()) {
			// Recurse tree
...
private:
	static inline cInt perp_dot(const IntPoint& a, const IntPoint& b) {
		return (-a.Y * b.X) + (a.X * b.Y);
	}

	static bool edge_collision(const IntPoint& a1, const IntPoint& a2, const IntPoint& b1, const IntPoint& b2, bool relaxed) {
	...
	static bool edge_poly_collision(const IntPoint& a1, const IntPoint& a2, const Path& poly) {
	...
	static bool splice_vertices(const Path& path1, const Path& path2, Path& result) {
		bool combined = false;
	...
	static Path keyhole_poly(const Path& poly, const Paths& holes) {
		Path keyedpoly = poly;
